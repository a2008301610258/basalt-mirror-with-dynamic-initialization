...
void KD_TREE::Nearest_Search(PointType point, int k_nearest,
                             ...
                             std::vector<float> &Point_Distance, double max_dist) {
...
void KD_TREE::Add_Point_Boxes(std::vector<BoxPointType> &BoxPoints) {
...
int KD_TREE::Delete_Point_Boxes(std::vector<BoxPointType> &BoxPoints) {
  int tmp_counter = 0;
  for (size_t i = 0; i < BoxPoints.size(); i++) {
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
...
void KD_TREE::BuildTree(KD_TREE_NODE **root, int l, int r,
  ...
  *root = new KD_TREE_NODE;
  ...
  // for (i=l;i<=r;i++){
  //     covariance[0] += (Storage[i].x - average[0]) * (Storage[i].x - average[0]); covariance[1] += (Storage[i].y - average[1]) * (Storage[i].y - average[1]); covariance[2] += (Storage[i].z - average[2]) * (Storage[i].z - average[2]);
  // }
...
void KD_TREE::print_tree(int index, FILE *fp, float x_min, float x_max,
                         float y_min, float y_max, float z_min, float z_max) {
  pthread_mutex_lock(&working_flag_mutex);
  print_treenode(Root_Node, index, fp, x_min, x_max, y_min, y_max, z_min,
                 z_max);
  pthread_mutex_unlock(&working_flag_mutex);
}

void KD_TREE::print_treenode(KD_TREE_NODE *root, int index, FILE *fp,
                             float x_min, float x_max, float y_min, float y_max,
                             float z_min, float z_max) {
  if (root == nullptr) return;
  Push_Down(root);
  fprintf(fp, "%d,%0.3f,%0.3f,%0.3f", index, root->point.x, root->point.y,
          root->point.z);
  fprintf(fp, ",%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f\n", x_min, x_max, y_min,
          y_max, z_min, z_max);
  switch (root->division_axis) {
    case 0:
      print_treenode(root->left_son_ptr, index, fp, x_min, root->point.x, y_min,
                     y_max, z_min, z_max);
      print_treenode(root->right_son_ptr, index, fp, root->point.x, x_max,
                     y_min, y_max, z_min, z_max);
      break;
    case 1:
      print_treenode(root->left_son_ptr, index, fp, x_min, x_max, y_min,
                     root->point.y, z_min, z_max);
      print_treenode(root->right_son_ptr, index, fp, x_min, x_max,
                     root->point.y, y_max, z_min, z_max);
      break;
    case 2:
      print_treenode(root->left_son_ptr, index, fp, x_min, x_max, y_min, y_max,
                     z_min, root->point.z);
      print_treenode(root->right_son_ptr, index, fp, x_min, x_max, y_min, y_max,
                     root->point.z, z_max);
      break;
    default:
      break;
